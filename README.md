...
- `environments/development.tfvars`: This file contains the variable values for the development environment.
- `environments/production.tfvars`: This file contains the variable values for the production environment.

### Terraform Workspaces & Remote Backend
Terraform workspaces allow you to manage multiple environments (e.g., development, staging, production) within the same configuration. Each workspace has its own state file, which allows you to create and manage resources independently in each environment.

To make it easier and more reliable to manage multiple environments, we can store the state files in a remote backend. This way, we can share the state files between different team members and avoid conflicts when working on the same infrastructure.

To create a remote backend for our Terraform configuration, we can use the `az-tfstate` project to create a remote backend in Azure. It will create a resource group, storage account, and container for storing the state files.

To make use of remote backends, we will use the `azurerm` backend to store the state files in an Azure Storage Account. This way, we can share the state files between different team members and avoid conflicts when working on the same infrastructure.

To use a remote backend, we need to add the following block to our `provider.tf` file:
...
Then we can proceed to run the workspace commands to create and manage the workspaces.

To create a new workspace, run the following command:
```bash
terraform workspace new <workspace_name>
```
To switch to an existing workspace, run the following command:
```bash
terraform workspace select <workspace_name>
```
To list all workspaces, run the following command:
```bash
terraform workspace list
```


So to deploy the infrastructure to a specific environment, we can run the following command:
...
Followed by:
```bash
terraform plan -out dev.tfplan -var-file=./environments/development.tfvars
```
```bash
